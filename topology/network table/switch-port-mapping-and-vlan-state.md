...
Default gateway must match VLAN subnet — DHCP and static IPs need correct gateway
VLAN tagging is how VMs access segmented network — Hypervisor NIC must be trunk-capable
Laptop Ethernet is trunk, not access port — Allows multiple VLAN access simultaneously
Proper segmentation requires both switch + firewall — Switch does Layer 2, firewall does Layer 3 policy
WiFi and Ethernet are separate paths — Laptop on WiFi = Home VLAN only; on Ethernet = all VLANs possible
Firewall policies enforce inter-VLAN rules — Switch can't prevent VLAN 20→60 access; firewall does
Sandbox isolation requires firewall rules, not just VLAN separation — Network-level containment
